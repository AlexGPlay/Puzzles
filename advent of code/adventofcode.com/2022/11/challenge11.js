...
function play(rounds, monkeys, divideWorryLevel, moduloWorryLevel = null) {
  for (let i = 0; i < rounds; i++) {
    if ((i + 1) % 1000 === 0) {
      console.log(i + 1, "/", rounds);
    }
    for (let monkey of monkeys) {
      for (let item of [...monkey.items]) {
        monkey.inspectedItems += 1;
        let worryLevel = monkey.operation(item);
        if (divideWorryLevel) worryLevel = Math.floor(worryLevel / 3);
        if (moduloWorryLevel) worryLevel %= moduloWorryLevel;
...
console.log(
  play(
    ...
    monkeys.map((monkey) => ({ ...monkey, items: [...monkey.items] })),
    false,
    monkeys.map((monkey) => monkey.testNumber).reduce(lcm)
  )
);
